...
class Client:
    ...
    def __init__(self, name=False, do_upload=True):
        ...
        self.print_log(['Starting Client', self.id])
        self.disconnect_others()
    ...
    def send_ssh_command(self, command, wait=1):
    ...
    def stop_remote_python(self):
        ...
        self.send_ssh_command('killall python', wait = 3)
    ...
    def remote_folder_exists(self, folder):
        ...
        :param folder: The folder to be tested (linux format)
        :return: Boolean
        """
        try:
    ...
    def delete_remote_folder(self, folder):
        """
        Function to delete a remote folder on the raspberry.

        For student use: No

        :param folder: Folder to be deleted (linux format)
        :return: None
        """
        command = 'rm -r ' + folder
        self.print_log(command)
        self.send_ssh_command(command, wait = 1)
    
    
    def delete_remote_file(self, file_path):
        """
        Function to delete a remote file on the raspberry.

        For student use: No

        :file folder: File to be deleted (linux format)
        :return: None
        """
    ...
    def make_remote_folder(self, folder):
        """
        Function to make a remote folder on the raspberry.
    ...
    def upload_files(self, verbose=False):
        """
        This function uploads all the files in PD2030 to the raspberry pi.
        ...
        if verbose: self.print_log('If this takes too long, consider disabling the file upload.')
